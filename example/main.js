...
const particles = Array.apply(null, Array(100)).map(() => new Entity(Particle, {Color: {color: '#9090CC'}, Transform: {position: [Math.random()*1000, Math.random()*1000]}, RigidBody: {speed: [0, 0]}, Circle: { radius: 1 }, Mass: {mass: 0.1}}))
...
const ecs = new Ecs(
    ...
    [new CanvasClearSystem(), new CircleCanvasSystem(), new RigidBodySystem(), new GravitySystem(), new BoundingBoxSystem(), new KeyboardMovementSystem()],
    [new KeyboardDashListener()],
    {
        canvas,
        config: {
            canvasColor: '#2E3440',
